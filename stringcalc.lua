	...
	if type(Formula) == "string" then
		...
		for M1, M2 in string.gmatch(Formula, "(%d*%.?%d*)(%D?)") do
			...
			elseif M1 ~= "" then
				...
				table.insert(Nums[#Nums], tonumber(M1))
			end
			...
			if M2 == "(" then
				...
				-- Add new stack
				local In = {}
				table.insert(Nums[#Nums], In)
				table.insert(Nums, In)
			elseif M2 == ")" then
				-- Close bracket
				if Var then
					-- End collection of variable and add to stack
				...
				if Nums[#Nums - 1][#Nums[#Nums - 1] - 1] == "," then
					...
					-- Interpret bracket before running function
					Nums[#Nums] = {Interpret(Nums[#Nums], LocalVars, LocalFuncs)}
				end
				...
				if type(Previous) == "function" or LocalFuncs[Previous] then
					...
					if type(Previous) == "number" then
						...
						table.insert(Nums[#Nums], Nums[#Nums][#Nums[#Nums]])
			...
			elseif M2 == "," then
				...
				Nums[#Nums][1] = Result
				table.insert(Nums[#Nums], M2)
				-- Add the new stack
				local In = {}
				table.insert(Nums[#Nums], In)
				table.insert(Nums, In)			elseif M2:find("%W") then
		...
		if Var then
			...
			table.insert(Nums[#Nums], LocalVars[Var] or math[Var] or LuaMathFuncs[Var] or Var)
		end
